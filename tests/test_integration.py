...
class TestLLMIntegration:
    ...
    def test_collate_command_help(self):
        ...
        assert "ARGS" in result.output  # Variable arguments for providers and query
        assert "--db" in result.output  # Database path option
        assert "--save" in result.output  # Save to database option
    ...
    def test_llm_command_execution(self, mock_claude_provider):
        ...
        runner = CliRunner()
        result = runner.invoke(main, ["llm", "claude", "Hello, how are you?"])
    ...
    def test_llm_command_with_streaming(self, mock_openai_provider):
        ...
        runner = CliRunner()
        result = runner.invoke(main, ["llm", "openai", "Hello", "--stream"])
...
class TestMCPIntegration:
    ...
    def test_mcp_command_simple(self, mock_mcp_client):
        ...
        mock_client_instance.ping.return_value = MCPResponse(
            result={"status": "pong"}, id=2
        )
        mock_mcp_client.return_value.__aenter__.return_value = mock_client_instance

        runner = CliRunner()
        result = runner.invoke(main, ["mcp", "http://localhost:8000", "ping"])
    ...
    def test_mcp_command_with_method(self, mock_mcp_client):
        ...
        mock_client_instance.send_message.return_value = MCPResponse(
            result={"tools": [{"name": "search"}, {"name": "calc"}]}, id=2
        )
        mock_mcp_client.return_value.__aenter__.return_value = mock_client_instance

        runner = CliRunner()
        result = runner.invoke(
            main, ["mcp", "http://localhost:8000", "--method", "tools/list"]
        )
    ...
    def test_mcp_convert_command(self, mock_translator):
        ...
        mock_translator_instance.get_suggestions.return_value = [
            "list tools",
            "ping server",
        ]
        ...
        runner = CliRunner()
        result = runner.invoke(main, ["mcp-convert", "list all available tools"])
