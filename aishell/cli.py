...
@click.option('--config', '-c', type=click.Path(exists=True), help='Use saved configuration file')
@click.option('--provider', '-p', type=click.Choice(['claude', 'openai', 'gemini', 'ollama', 'opus', 'haiku']), help='LLM provider for task translation')
@click.option('--fallback', type=click.Choice(['claude', 'openai', 'gemini', 'ollama', 'opus', 'haiku']), help='Fallback provider if primary fails')
@click.option('--save-config', type=click.Path(), help='Save generated configuration to file')
@click.option('--output', '-o', type=click.Path(), help='Output file for extracted data')
@click.option('--headless/--no-headless', default=True, help='Run browser in headless mode')
@click.option('--browser', type=click.Choice(['chromium', 'firefox', 'webkit']), default='chromium', help='Browser type')
def navigate(url, task, config, provider, fallback, save_config, output, headless, browser):
    ...
    async def run_navigation():
        ...
        from usecases.webscraping import WebNavigator, LLMNavigator, ScrapingConfig, ConfigLibrary
        from aishell.utils import get_env_manager
        ...
        if task or (config and provider):
            ...
            if provider == 'opus':
                ...
                provider_name = provider or env_manager.get_var('DEFAULT_LLM_PROVIDER', 'claude')
                model_override = None
        ...
        async with WebNavigator(
            ...
            if config:
                ...
                with console.status("[yellow]Executing navigation...[/yellow]", spinner="dots"):
                    result = await navigator.execute_config(scraping_config)

            elif task:
                # Mode 1: Free-form task with LLM
                if not llm_provider_instance:
                    console.print("[red]Error:[/red] LLM provider required for task-based navigation")
                    return
                ...
                with console.status("[yellow]Generating navigation plan...[/yellow]", spinner="dots"):
                    save_path = Path(save_config) if save_config else None
                    result = await llm_nav.execute_task(task, url, save_config=save_path)
                ...
                console.print("[red]Error:[/red] Either --task or --config must be provided")
                return
            ...
            if result:
                if result.success:
                    ...
                    console.print(f"[blue]Actions executed:[/blue] {result.actions_executed}")
                    ...
                    if result.data:
                        ...
                        panel = Panel(syntax, title="[green]Data[/green]", border_style="green", padding=(1, 2))
                        console.print(panel)
                    ...
                    if result.metadata:
                        ...
                        console.print(f"  [dim]Title:[/dim] {result.metadata.get('title', 'N/A')}")
